...
export async function POST(request: Request) {
  try {
    ...
    console.log('Données brutes reçues (direct):', requestText.substring(0, 500) + '...');
    ...
    console.log('Données reçues pour importation directe:', {
      name: data.name,
    ...
    // Créer un dictionnaire des matériaux pour référence rapide
    ...
    if (data.materials && data.materials.length > 0) {
      for (const material of data.materials) {
        try {
          ...
          console.log('Matériau créé:', newProduct.name);
        } catch (error) {
    ...
    // Traiter les services (prestations) et les répartir dans les catégories
    const processedServices: any[] = [];
    if (data.services && data.services.length > 0) {
      // Répartir les services dans les catégories
      for (let i = 0; i < data.services.length; i++) {
        ...
        try {
          ...
          console.log('Service créé:', newProduct.name, 'dans la catégorie', category.name, 'avec', materialsToAssociate.length, 'matériaux');
        } catch (error) {
    ...
    console.log('Importation directe terminée:', {
      catalogId: catalog.id,
    ...
    console.error('Erreur globale lors de l\'importation directe du catalogue:', error);
    return NextResponse.json(
      { 
